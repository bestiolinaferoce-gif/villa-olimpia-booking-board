...
/**
 * Upload generico di un allegato su Vercel Blob (documenti, ricevute, ecc.).
 * multipart/form-data, campo "file". Richiede BLOB_READ_WRITE_TOKEN.
 */
export async function POST(req: NextRequest) {
  ...
  const pathname = `allegati/${Date.now()}-${safeName}`;
  const contentType = file.type || "application/octet-stream";

  try {
    const body = Buffer.from(await file.arrayBuffer());
    const blobRes = await fetch(`https://blob.vercel-storage.com/${encodeURI(pathname)}`, {
      method: "PUT",
      headers: {
    ...
    const data = (await blobRes.json()) as { url: string; contentType?: string };
    return NextResponse.json({
      ok: true,
      attachment: {
    ...
    return NextResponse.json({ ok: false, error: "upload_exception", detail: String(err) }, { status: 500 });
  }
}
